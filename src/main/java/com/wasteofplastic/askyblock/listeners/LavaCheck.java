...
public class LavaCheck implements Listener {
    ...
    public void onCleanstoneGen(BlockFromToEvent e) {
        ...
        if (Settings.acidDamage > 0) {
            ...
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                if ((prev.equals(Material.WATER) || LegacyMaterial.isLava(prev)) && to.getType().equals(Material.STONE)) {
                    to.setType(prev);
    ...
    public void onCobbleGen(BlockFromToEvent e){
        ...
        if (LegacyMaterial.isLava(b.getType()) || LegacyMaterial.isWater(b.getType())) {
    ...
    public boolean generatesCobble(Block block, Block toBlock){
        Material mirrorID1 = LegacyMaterial.isWater(block.getType()) ? Material.LAVA : Material.WATER;
        Material mirrorID2 = LegacyMaterial.isWater(block.getType()) ? LegacyMaterial.STATIONARY_LAVA : LegacyMaterial.STATIONARY_WATER;
        for (BlockFace face: FACES) {
            Block r = toBlock.getRelative(face);
