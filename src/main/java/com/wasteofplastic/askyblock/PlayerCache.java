...
public class PlayerCache {
    ...
    public int getMaxTeamSize(Player player){
        return getMaxInt(player, "askyblock.team.maxsize", Settings.maxTeamSize);
    }

    public int getMaxHomes(Player player){
        return getMaxInt(player, "askyblock.", Settings.maxHomes);
    }

    public boolean isDisabledWorldBorder(final UUID playerUUID) {
        addPlayer(playerUUID);
        return playerCache.get(playerUUID).isDisabledWorldBorder();
    }

    public void setDisabledWorldBorder(final UUID playerUUID, boolean disabledWorldBorder) {
        addPlayer(playerUUID);
        playerCache.get(playerUUID).setDisabledWorldBorder(disabledWorldBorder);
    }

    private int getMaxInt(Player player, String permission, int def){
        int maxInt = def;

        Matcher matcher;
        for (PermissionAttachmentInfo perms : player.getEffectivePermissions()) {
            if((matcher = Pattern.compile(permission + "\\.(.*)").matcher(perms.getPermission())).matches()){
                String maxIntString = matcher.group(1);

                try{
                    maxInt = Math.max(maxInt, Integer.valueOf(maxIntString));
                }catch(IllegalArgumentException ex){
                    if(maxIntString.equals("*")) {
                        maxInt = def;
                        break;
                    }
                }
